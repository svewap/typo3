 ...
 */
class PageTreeNode implements \ArrayAccess
{
    use PropertyTrait;
    ...
    public function __construct(array $properties)
    {
        foreach ($properties as $propertyName => $propertyValue) {
            if (in_array($propertyName, $this->specialPropertyNames)) {
                if ($propertyName === '_TRANSLATION_SOURCE' && !$propertyValue instanceof PageTreeNode) {
                    $this->specialProperties[$propertyName] = new PageTreeNode($propertyValue);
                } else {
    ...
    {
        return $this->specialProperties['_language'] ?? $this->specialProperties['_PAGES_OVERLAY_LANGUAGE'] ?? $this->properties['language_tag'];
    }
    ...
    public function getTranslationSource(): ?PageTreeNode
    {
